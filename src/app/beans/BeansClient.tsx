...
import { ORIGINS, ORIGIN_MAP, ROAST_LABEL, ROAST_COLOR } from '@/data/beans'
...
export default function BeansClient({ initialBeans }: BeansClientProps) {
  // Data is provided by the Server Component — no client-side fetch needed.
  ...
  // Memoize expensive derivations — beans is stable (from server), only recompute when filters change.
  const ALL_NOTES = useMemo(() => {
    const freq = new Map<string, number>()
    beans.forEach(b => b.notes.forEach(n => freq.set(n, (freq.get(n) ?? 0) + 1)))
    return ['전체', ...[...freq.entries()].sort((a, b) => b[1] - a[1]).map(([n]) => n)]
  }, [beans])

  const NOTE_OPTIONS = useMemo(() => ALL_NOTES.filter(n => n !== '전체'), [ALL_NOTES])
  ...
  const matchingNotes = useMemo(
    () => normalizedNoteQuery
      ...
      : NOTE_OPTIONS,
    [NOTE_OPTIONS, normalizedNoteQuery],
  )

  const visibleNotes = useMemo(
    () => normalizedNoteQuery || showAllNotes ? matchingNotes : matchingNotes.slice(0, COLLAPSED_NOTE_COUNT),
    [matchingNotes, normalizedNoteQuery, showAllNotes],
  )
  ...
  const filtered = useMemo(
    ...
    [beans, activeOrigin, activeNotes, normalizedNoteQuery],
  )
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        // Batch all intersecting entries into one setState call.
