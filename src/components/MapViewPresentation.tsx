...
export default function MapViewPresentation(state: MapViewState) {
  const {
    allCafes,
    filteredCafes,
    mapCafes,
  ...
  return (
    <div className="map-font relative h-dvh overflow-hidden">
      <Sidebar
        ...
        onCafeSelect={(cafe) => selectCafe(cafe, 'sidebar')}
      ...
      <div className="absolute inset-0">
        ...
        <div className="pointer-events-none absolute inset-x-0 top-[84px] z-20 flex justify-center px-4 md:top-4">
          <button
            ...
            onClick={() => handleSearchCurrentMap((bounds) => {
              if (selectedCafe && !isCafeInsideBounds(selectedCafe, bounds)) {
                clearSelection()
              }
            })}
        ...
        {!mobileSheetExpanded && (
          <MobileCafeCarousel
            ...
            onSelect={(cafe) => selectCafe(cafe, 'carousel')}
            onRequestLocation={() => {
              setLocationPermissionError(null)
              setLocationPermissionModalOpen(true)
            }}
